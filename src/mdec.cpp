...
static void idct1(long *block)
{
	long val = RANGE(DESCALE(block[0], PASS1_BITS+3));
	int i;
	for(i=0;i<DCTSIZE2;i++) block[i]=val;
}


void idct(long *block,int k)
{
...
unsigned short* rl2blk(long *blk,unsigned short *mdec_rl);
void iqtab_init(long *iqtab,unsigned char *iq_y);
...
long iq_y[DCTSIZE2],iq_uv[DCTSIZE2];
...
void mdec1DMAInterrupt()
{
	...
	if (HW_DMA1_CHCR & 0x01000000) {
		...
		psxRegs->intCycle[5+24+1] = (PsxClock/75);
...
void psxDma0(u32 adr, u32 bcr, u32 chcr) {
	...
	psx4all_prof_start(PROFILER_DMA0);
	...
	DMA_INTERRUPT(0);
	psx4all_prof_end(PROFILER_DMA0);
}



INLINE void MDECOUTDMA_INT(u32 uCycles)
{
	psxRegs->interrupt       |= 0x02000000;
	psxRegs->intCycle[5+24]    = psxRegs->cycle;
	psxRegs->intCycle[5+24+1]  = uCycles;
}
...
void psxDma1(u32 adr, u32 bcr, u32 chcr) {
	...
	psx4all_prof_start(PROFILER_DMA1);
	size = (bcr>>16)*(bcr&0xffff);
    	image = (u16*)PSXM16(adr);

	if (mdec.command&0x08000000) {
		MDECOUTDMA_INT(size / 4);
		//MDECOUTDMA_INT((u32)((size / 128) / PsxCycleMult));
		size = size / ((16*16)/2);
	...
	psxRegs->cycle += (uDefaultPsxMult*uPsxClockBase*16) / (320*240*30);

	mdec.status|= MDEC_BUSY;
	psx4all_prof_end(PROFILER_DMA1);
}
...
#ifdef FIXED
#define	MULR(a)		((((long)0x0000059B) * (a)) >> 10)
#define	MULG(a)		((((long)0xFFFFFEA1) * (a)) >> 10)
#define	MULG2(a)	((((long)0xFFFFFD25) * (a)) >> 10)
#define	MULB(a)		((((long)0x00000716) * (a)) >> 10)
#else
#define	MULR(a)		((long)((float)1.40200 * (a)))
#define	MULG(a)		((long)((float)-0.3437 * (a)))
#define	MULG2(a)	((long)((float)-0.7143 * (a)))
#define	MULB(a)		((long)((float)1.77200 * (a)))
#endif
...
void yuv2rgb15(long *blk,unsigned short *image) {
	int x,y;
	long *Yblk = blk+DCTSIZE2*2;
	int Cb,Cr,R,G,B;
	long *Cbblk = blk;
	long *Crblk = blk+DCTSIZE2;
